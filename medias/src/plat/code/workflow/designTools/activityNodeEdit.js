...
function saveSheetEdit() {
    ...
    var spxSort = "";
    var spxPrint = "";
    if (_top.workflowShowSpx && control == "EXAMINE") {
        spxName = $.getEle("ModelActivityNode", "spxName").val() || "";
        spxSort = $.getEle("ModelActivityNode", "spxSort").val() || "";
        spxPrint = $.getEle("ModelActivityNode", "spxPrint").val() || "0";
    }
...
function roleSelectCallback(id, name, code, roleType, roleTypeName, roleMade) {
    ...
    modelActivityNode.setValue("roleMade", roleMade);
    var typeName = "";
    if (roleType == "1") {
        typeName = "业务角色";
    } else if (roleType == "2") {
...
$(function () {
    ...
    var SheetTrView = BaseElementView.extend({
        ...
        renderEditMode: function () {    //实现渲染接口
            ...
            if (_top.workflowShowSpx) {
                html += "<td><input type='text' class='i_text' data-model='" + this.model.get("ModelName") + "' data-property='spxName' value='" + this.model.get("spxName") + "'></td>" +
            ...
            html += "<td><a class='editEle'>修改</a><a class='delete'>删除</a></td>";
            $(this.el).html(html);
        },
    ...
    var SheetTableView = BaseTableView.extend({
        getControlHtml: function () { //实现控制区域渲染接口
            ...
            controlStr += '<th><b>*</b>序号</th>' +
                ...
                '<input  type="text" class="i_text" data-model="ModelActivityNode"  data-property="sheetSort"/>' +
                '</div></td>';
            controlStr += '<th><b>*</b>标题</th>' +
                '<td><div class="element_box">' +
                '<input  type="text" class="i_text" data-model="ModelActivityNode"  data-property="sheetTitle"/>' +
                '</div></td>';
            controlStr += '<th><b>*</b>选择表单</th>' +
                '<td><div class="element_box">' +
                '<select  class="i_select" data-model="ModelActivityNode"  data-property="sheetId"/>' +
                '</div></td>';
            controlStr += '<th><b>*</b>表单功能</th>' +
                '<td><div class="element_box">' +
                '<select  class="i_select" data-model="ModelActivityNode"  data-property="sheetMode"/>' +
                '</div></td>';
            ...
            if (_top.workflowShowSpx) {
                controlStr += '<tr>'
                controlStr += '<th class="spxItem" >审批项名称</th>' +
                    '<td class="spxItem" ><div class="element_box">' +
                    '<input  class="i_text spxItem" data-model="ModelActivityNode"  data-property="spxName"/>' +
                    '</div></td>';
                controlStr += '<th class="spxItem">审批项序号</th>' +
                    '<td class="spxItem" ><div class="element_box">' +
                    '<input  class="i_text spxItem" data-model="ModelActivityNode"  data-property="spxSort"/>' +
                    '</div></td>';
                controlStr += '<th class="spxItem" >打印审批项</th>' +
                    '<td class="spxItem" ><div class="element_box">' +
                    '<select  class="i_select spxItem" data-model="ModelActivityNode"  data-property="spxPrint">' +
        ...
        getTheadHtml: function () {  //实现表头区域渲染接口
            ...
            if (_top.workflowShowSpx) {
                ...
                theadstr = "<thead>" +
                    "<th width='5%'>序号</th><th width='30%'>标题</th><th width='30%'>所属表单</th>" +
                    "<th width='15%'>表单控制</th><th >操作</th>" +
                    "</thead>";
            }
            return theadstr;
        },
        ...
        addNewItem: function () {
            ...
            var spxPrint = "";
            if (_top.workflowShowSpx) {
                spxName = control == "EXAMINE" ? $.getEle("ModelActivityNode", "spxName").val() : "";
                spxSort = control == "EXAMINE" ? $.getEle("ModelActivityNode", "spxSort").val() : "";
                spxPrint = control == "EXAMINE" ? $.getEle("ModelActivityNode", "spxPrint").val() : "0";
            }
            if (!sheet_id || sheet_id.toString().trim() === "") {
                rxMsg(RxMsgType.WARNNING, "请选择表单");
                return;
            }
            if (!sort || sort.toString().trim() === "") {
                rxMsg(RxMsgType.WARNNING, "请填写序号");
                return;
            }
            if (!title || title.toString().trim() === "") {
                rxMsg(RxMsgType.WARNNING, "请填写标题");
                return;
            }
            var view = this;
    ...
    var ButtonTableView = BaseTableView.extend({
        ...
        getTheadHtml: function () {  //实现表头区域渲染接口
            return "<thead>" +
                "<th width='5%'>序号</th>" +
                "<th width='15%'><b style='color:red'>*</b>名称</th>" +
                "<th width='12%'><b style='color:red'>*</b>code</th>" +
                "<th width='10%'>图标</th>" +
                "<th width='13%'>函数名</th>" +
                "<th>排序号</th>" +
                "<th>显示条件</th>" +
