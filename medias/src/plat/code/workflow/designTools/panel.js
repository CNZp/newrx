var panel = null;
var freshId = GetQueryString("freshId");
...
function save() {
    ...
    var type = flow.type;
    if (type == null || type == "") {
        rxMsg(RxMsgType.WARNNING,"流程类别不可为空");
        return false;
    }
        ...
        success: function (ar) {
            if (ar.success) {
                ...
                parent.reloadWfTree(function(){
                   gotoLocation("/workflow/designTools/panel?freshId="+ar.data);
                })
            } else {
...
$(function () {
    $("body").ajaxStart(function () {
        index = layer.msg("服务器正在处理您的请求，请稍候...");
    }).ajaxStop(function () {
        layer.close(index);
    });
});