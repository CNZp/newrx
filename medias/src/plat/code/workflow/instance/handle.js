...
$(function () {
    ...
    $("#viewFile").click(function () {
        if (!$("#fj_id").val()) {
            var uuid = forms.uuid();
            $("#fj_id").val(uuid);
            openStack(window, "资料附件上传", ["550px", "400px"], "/attachment/wfAttachment?fj_id=" + uuid);
        } else {
            openStack(window, "资料附件上传", ["550px", "400px"], "/attachment/wfAttachment?fj_id=" + $("#fj_id").val());
        }
    });
    //点击提交按钮触发事件
    $("#sure").click(function () {
        ...
        param.sureFunc($("#description").val(), $("#fj_id").val(),param.layerMsg);
        $("#sure").prop("disabled", false);
    });
