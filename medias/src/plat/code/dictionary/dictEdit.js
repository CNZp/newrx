var subTableView; //声明字典项view类
var ModelSubDict; //声明字典项model类
var modelDict; //声明字典model类实例
var ModelDict; //声明字典model类
//操作类型标志位
    ...
    var SubTableView = BaseTableView.extend({
        ...
        getTheadHtml: function () {  //实现表头区域渲染接口
            return "<thead>" +
                ...
                "<th style='width:25%'>上级字典项编码</th>  " +
                "<th style='width:10%'>排序号</th>" +
                "<th style='width:15%'>字典项扩展</th>" +
                "</thead>";
        },
...
function setDictCode() {
    ...
    var dictCode = $.getEle("ModelDict", "dictCode").val();
    var pdictCode = $.getEle("ModelDict", "pdictCode").val();
