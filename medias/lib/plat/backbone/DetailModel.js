            ...
            if (relations.length > 0) {
                for (var n = 0; n < relations.length; n++) {
                    ...
                    if (relations[n].type == Backbone.HasMany) {
                        if (model.get(tempkey).length > 0) {
                            var itemsstr = "";
                            //过滤无效关系对象，使用unvalidNum标记。
                            //Fixme:如关系对象集合重新排序，或原数据集合数据包含unvalid数据，此处也存在问题。
                            //Todo:建议在子对象中打上标记，出现无标记内容，必属新增，出现有标记内容，比较两者。
                            //Todo:对于特殊业务含义的删除操作，会导致原始数据集合其他数据变化，建议补充表单展示用关系数据集，用与判断变更。
                            var unvalidNum = 0;
                            for (var i = 0; i < model.get(tempkey).length; i++) {
                                if( model.get(tempkey).at(i).get("sfyx_st") == "UNVALID"){
                                    unvalidNum++;
                                    continue;
                                }
                                var tresult;
                                var obj = historyObj[tempkey][i-unvalidNum];
                                if (obj) {
