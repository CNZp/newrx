...
var AttachmentListView = Backbone.View.extend({
    ...
    getControlHtml: function () {
        ...
        if (view.state == "ck") {
            ...
            if (view.minNum != null && typeof(view.minNum) === "number") {
                controlstr += "<p style='color: #717171'><em style='float: left'>（至少上传" + view.minNum + "份）</em></p>" + "<ul class='action_button to_right ' minnum='" + view.minNum + "'><li><a href='#none' class='addFileItem'><i class='iconfont '>&#xe8ab;</i>上传附件</a></li>";
            } else {
    ...
    render: function () {
        ...
        if (olist.length > 0 || !dicts) {
            ...
            headhtml += '<a href=\"javascript:void(0);\" title=\"展开\"><img alt=\"\" src=' + RX.handlePath("/medias/style/plat/image/common/Arrow1.gif") + '></a>' +
                "</div>";
        ...
        if (dicts && dicts.length > 0) {
            //是否页面配置
            var pageFlag = false;
            for (var i = 0, maxLength = dicts.length; i < maxLength; i++) {
                if (dicts[i].remark) {
                    pageFlag = true;
                    break;
                }
            ...
            $.each(dicts, function (index, value) {
                ...
                if (pageFlag) {
                    ...
                    //警保前期在属性中配置
                    if (view.minNum) {
                        if (view.minNum["zd" + value.code] && view.minNum["zd" + value.code] > 0) {
                ...
                if (view.state == "xz" && minNum > 0) {
                    headhtml += "<p style='color:#717171;'><em style='float: left'>至少上传" + minNum + "份</em></p>";
                }
                headhtml += "<a href='javascript:void(0);' title=\"展开\"><img alt=\"\" src=\"" + RX.handlePath("/medias/style/plat/image/common/Arrow1.gif") + "\"/></a>" +
                    "</div>";
                var headdiv = $(headhtml);
...
var AttachmentListViewHtml5 = Backbone.View.extend({
    ...
    render: function () {
        ...
        if (olist.length > 0 || !dicts) {
            var headhtml = "<div class=\"attachment_header\" minNum='0'>" +
                "<h1>未分类资料(<em>" + olist.length + "</em>)" + "</h1>";
            headhtml += '<a href=\"javascript:void(0);\" title=\"展开\"><img alt=\"\" src=' + RX.handlePath("/medias/style/plat/image/common/Arrow1.gif") + '></a><a href="javascript:void(0);" class="addAttachment1">添加附件</a>' +
                "</div>";
            var headdiv = $(headhtml);
        ...
        if (dicts && dicts.length > 0) {
            $.each(dicts, function (index, value) {
                ...
                headhtml += "<a href='javascript:void(0);' title=\"展开\"><img alt=\"\" src=\"" + RX.handlePath("/medias/style/plat/image/common/Arrow1.gif") + "\"/></a><a href='javascript:void(0);' class='addAttachment1' fileNo='" + value.code + "'>添加附件</a>" +
                    "</div>";
                var headdiv = $(headhtml);
...
var AttachmentView = Backbone.View.extend({
    ...
    render: function () {
        ...
        if (ftype == "docx" || ftype == "doc") {
            src = "/medias/style/plat/image/attachment/word.png";
        } else if (ftype == "xls" || ftype == "xlsx") {
            src = "/medias/style/plat/image/attachment/excel.png";
        } else if (ftype == "ppt" || ftype == "pptx") {
            src = "/medias/style/plat/image/attachment/ppt.png";
        } else if (ftype == "ppt" || ftype == "pptx") {
            src = "/medias/style/plat/image/attachment/ppt.png";
        } else if (ftype == "rar" || ftype == "zip" || ftype == "7z") {
            src = "/medias/style/plat/image/attachment/zip.png";
            ...
            src = "/medias/style/plat/image/attachment/pt.png";
        }
        var html = "<a href='javascript:void(0);'>" +
            "<img alt=\"\" onerror=\"onerror=null;src='" + RX.handlePath('/medias/style/plat/image/attachment/pt.png') + "'\"" + " src=\"" + RX.handlePath(src) + "\"/>" +
...
var AttachmentDivView = Backbone.View.extend({
    ...
    render: function () {
        ...
        if (view.state == "xz") {
            var dthtml = "<dt><img src='" + RX.handlePath("/medias/style/plat/image/attachment/enclosure.png") + "'><a class='addFileItem' href='javascript:;' title='" + view.uploadName + "'>" + view.uploadName + "</a>";
            if (minNum > 0) {
            ...
            var dthtml = "<dt><img src='/medias/style/plat/image/attachment/enclosure.png'><span>" + view.uploadName + "</span></dt>";
            $(dl).append(dthtml);
        }
...
var AttachmentTableView = Backbone.View.extend({
    ...
    getControlHtml: function () {
        ...
        if (view.state == "ck") {
            ...
            if (minNum > 0) {
                controlstr += "<p><em style='float: left'>(至少上传" + minNum + "份)</em></p>";
            }
    ...
    fileChange: function () {
        var view = this;
        $("#uploadForm").attr("action", "/" + RX.config.ctxPath + "/attachment/mulupload?uuid=" + view.uuid);
...
var AttachmentTrView = Backbone.View.extend({
    ...
    //下载事件响应方法
    downloadFile: function () {
        window.open("/" + RX.ctxPath + "/attachment/download?id=" + this.model.get("id"));
    }
});
...
var AttachmentSingleView = Backbone.View.extend({
    ...
    render: function () {
        ...
        $(dl).css("float", "left");
        if (view.state == "xz") {
            $(dl).append("<dt style='float:left;'><img src=" + RX.handlePath('/medias/style/plat/image/attachment/enclosure.png') + "><a class='addFileItem' href='javascript:;' title='" + view.uploadName + "'>" + view.uploadName + "</a></dt>");
        }
        $.each(view.collection.models, function (index, value) {
            view.index++;
            var dd = view.getNewFileView(value, 'renderEditMode', view.index, view.state).render().el;
...
var ImageUploadView = Backbone.View.extend({
    ...
    initialize: function (options) {
        ...
        if (options.canDelete != null && options.canDelete != "") {
    ...
    events: {
        ...
        'click li.deleteFile': 'deleteFile',
        'click a.addZpItem': 'addNewItem',
        'change input.addFile': "headFileChange"
    },
    //依据uuid请求数据方法
    //主渲染方法
    render: function () {
        ...
        $(view.el).removeClass().addClass(view.className);
        if (view.zpid) {
            $(view.el).prev().val(view.zpid);
            $(view.el).append("<form id='uploadForm2' enctype='multipart/form-data' method='POST'>" +
                '<a herf="javascript:;">' +
                '<input type="file" class="addFile headImage" name="filedata" id="uploadFile">' +
                "<img class='zpimage' style='width: 100%' alt=\"\" onerror=\"onerror=null;src='" + RX.handlePath('/medias/style/plat/image/common/photo.jpg') + "'\"" + " src=\"" + RX.handlePath('/attachment/getImage?id=') + view.zpid + "\"/>" +
                '</a></form>'
            );
        } else {
            $(view.el).append("<form id='uploadForm2' enctype='multipart/form-data' method='POST'><a herf='javascript:;'>" +
                "<input type=\"file\" class=\"addFile headImage\" name=\"filedata\" id=\"uploadFile\">" +
                '<img class="zpimage" style="width:100%"  src=' + RX.handlePath("/medias/style/plat/image/common/photo.jpg") + '/>' +
                "</a></form>");
        }
        if (view.canDelete && !$(view.el).parent().find('i').hasClass('deleteImage') && view.state !== 'ck') {
            $(view.el).parent().append('<p style="text-align: center">' +
                '<i style="cursor:pointer" class="iconfont deleteImage">&#xe606;</i>' +
                '</p>');
    ...
    headFileChange: function () {
        var view = this;
        $("#uploadForm2").attr("action", "/" + RX.config.ctxPath + "/attachment/upload?uuid=" + view.zpid);
        $("#uploadForm2").ajaxSubmit(function (ar) { //表单提交，根据后台Action返回的数据进行相关的操作
            view.zpid = JSON.parse(ar).zpId;
            view.render();
        });
    },
    deleteFile: function () {
        var view = this;
        if (view.zpid) {
            $.post("/attachment/delAttachment", {idstr: view.zpid}, function (ar) {
                if (ar.success) {
                    view.zpid = null;
                    view.render();
                }
...
var CysbUploadView = Backbone.View.extend({
    ...
    render: function () {
        ...
        if (view.zpid != "") {
            ...
            $(view.el).append("<a herf='javascript:;' class='addZpItem'><img class='zpimage' style='width:100%;height:auto;' src='/" + RX.ctxPath + "/medias/plat/image/common/photo.jpg'/></a>");
        }
    },
    //增加新的个体数据时，数据处理与页面渲染触发方法
    addNewItem: function () {
