...
MR.prototype = {
    ...
    defaultPropertyJson: {
        ...
        dateConfig: {
            ...
            maxDateExtend: null,//字符串，"{d:-1}"最小时间附加配置，比如最小时间-1d等， M（月），d（天） -：表示减少多少天，正数表示+多少天
            minDate: null, //最小日期，可为具体的日期字符串，也可为渲染最小日期的字段，也可为“sysdate”表示以系统时间为限。
            minDateExtend: null,  //字符串，"{d:-1}"最小时间附加配置，比如最小时间-1d等， M（月），d（天） -：表示减少多少天，正数表示+多少天
        ...
        if (proJson.spanShow) {
            ...
            if ($(t).hasClass("disabled") && !$(t).is(":hidden")) {
                if (tagName == "INPUT" && $(t).prop("type") == "text") {
                    ...
                    $(t).hide();
                    if (isDocEl) {
                        $(t).after("<font class='span_show_ellipsis'>" + replaceSymbols($(t).val()) + "</font>");
                    } else {
                    ...
                    if (isDocEl) {
                        $(t).after("<font class='span_show_ellipsis'>" + replaceSymbols(text) + "</font>");
                    } else {
            ...
            if (value2.type == "file") {
                    ...
                    if (minNum > 0) {
                        ...
                        if (num < minNum) {
                            ...
                            if ($(t).next().find(".err").length == 0) {
                                $(t).next().find("p").makeImgTip($(t), "附件数目不够");
                                $(t).next().find("p").append('<span class="err" ><i class="iconfont">&#xe603;</i></span>');
                            }
                        }
                    }
                    $(t).next().find(".attachment_header").each(function (i, t2) {
                        ...
                        if (minNum > 0) {
                            ...
                            if (parseInt(num) < minNum) {
                                if (!$(t2).find("p").next().hasClass('err')) {
                                    $(t2).makeImgTip($(t), "附件数目不够");
                                    $(t2).find("p").append('<span class="err" ><i class="iconfont">&#xe603;</i></span>');
                                }
                                result = false;
                            }
                        }
                ...
                //表格类型验证
                    ...
                    if ($.inArray("notNull", value2.rules.checkSave) != -1) {
                        if ($(t).val == null || $(t).val() == "") {
                            result = false;
                            $(t).parent().find(".zpimage").attr("src", RX.handlePath("/medias/style/plat/image/common/photo_error.jpg"));
                        }
                    }
                }
    ...
    renderProperty: function (t, model) {
        ...
        if (value2.type == "date") {
            ...
            if (value2.dateConfig.maxDate) {
                if (isDateTime(value2.dateConfig.maxDate)) {
                    ...
                    if (value2.dateConfig.maxDateExtend) {
                        json += "," + value2.dateConfig.maxDateExtend + "";
                    }
                    json += ")}'";
                }
            }
            ...
            if (value2.dateConfig.minDate) {
                if (isDateTime(value2.dateConfig.minDate)) {
                    ...
                    if (value2.dateConfig.minDateExtend) {
                        json += "," + value2.dateConfig.minDateExtend + "";
                    }
                    json += ")}'";
                }
            }
            ...
            if (!jqueryS.hasClass("doc_date") && !jqueryS.hasClass("disabled")) {
                //非文本日期，渲染时间控件
        ...
        if (value2.type == "layer") {
            if (value2.layerConfig.canDelete) {
                if (!jqueryS.hasClass("doc_layer") && !jqueryS.hasClass("disabled")) {
                    ...
                    $t.find(".popup").click(function () {
                        jqueryS.trigger("click", ["popup"]);
                    });
                }

            }
            else {
                if (!jqueryS.hasClass("doc_layer") && !jqueryS.hasClass("disabled") && !value2.display == false) {
                    var $t = $('<div class="auto_ico_box ico_1"></div>');
                    $t.append('<span class="popup" title="选择"><i class="iconfont">&#xe662;</i></span>');
                    jqueryS.parent().addClass("ele_1").append($t);
                    //选择事件绑定
            ...
            if (!value2.layerConfig.canInput) {
                jqueryS.prop("readonly", true);
            }
            jqueryS.unbind("click.ModelEvent");
            if (value2.layerConfig.type == "frame") {
                jqueryS.bind("click.ModelEvent", function (event, param) {
                    if (param == "popup" || !value2.layerConfig.canInput) {
                        var urldata = "";
                ...
                jqueryS.bind("click.ModelEvent", function (event) {
                    ...
                    $el.find("._treeAidFrame").attr("src", RX.handlePath(url));
        ...
        if (value2.type == "file") {
            if (typeof(value2.fileConfig) != "undefined") {
                ...
                if (value2.fileConfig.type != "image" && value2.fileConfig.type != "detailImage") {
                ...
                //todo
                else if (value2.fileConfig.type == "detailImage") {
                    view = new DetailImageUploadView({
                        zpid: keyValue,
                        state: fjState,
                        collection: new AttachmentCollection(),
                        el: $(t).next(),
                    ...
                    view = new ImageUploadView({
                        canDelete: value2.fileConfig.canDelete,
                        zpid: keyValue,
                        state: fjState,
                        collection: new AttachmentCollection(),
                        el: $(t).next(),
        ...
        //联想输入处理
        if (value2.canThink) {
            this._handleThinkInput(t, value2, model);
        }
        ...
        if (jqueryS.val() == null || jqueryS.val() == "") {
            if (value2.defaultValue) {
                ...
                keyValue = value2.defaultValue;
                if (tagName === "SELECT") {
                    setSelectVal(jqueryS, value2.defaultValue)
                } else {
        ...
        jqueryS.bind("blur.ModelEvent", function () {
            ...
            if (jqueryS.prop("tagName") == "INPUT" && !jqueryS.hasClass("TextBoxErr")) {
                jqueryS.prop("title", jqueryS.val());
            }
        });
    ...
    //处理联想输入
    _handleThinkInput: function (t, value2, model) {
        //去除change事件，change事件联想内部处理，不采用原生的
        //前置修改一些处理
        //前置修改一些处理，如果linkId存在解决change事件，如果不是，不去除，可能不是选中联想列表中的数据的
        if (value2.linkId && value2.changeFunc) {
            $(t).unbind("change.ModelEvent");
        }
        //创建窗口
        ...
        function createThinkIframe() {
            ...
            function addIframeContent() {
                ...
                setTimeout(function () {
                    ...
                    frameDocument.write(addContent.join(""));
                    frameDocument.close();
        ...
        function addThinkIframeEvent() {
            ...
            //复制值需不需要考虑？？
            $(t).bind("keyup.ModelEvent1", function (event) {
                    var $tvalue = $.trim($(t).val());  //去除左右空格
                    if ($tvalue) {
                            ...
                            timeoutId = setTimeout(function () {
                                if (!doc) {
                                    ...
                                    if (!docParam) {
                                        ...
                                        thinkDiv = $(".thinkDiv");
                                        $(window).data("thinkParam", {doc1: doc, thinkUl: thinkUl, thinkDiv1: thinkDiv});
                                    } else {
                                        doc = docParam.doc1;
                                ...
                                function addThinkLiEvent() {
                                    ...
                                    $(".thinkUl", doc).on("click", "li", function () {
                                        ...
                                        if (value2.linkId) {
                                            model.setValue(value2.linkId, linkId);
                                        }
                                        thinkDiv.hide();
                                        if (value2.changeFunc) {
                                            eval(value2.changeFunc + "(model,linkName,linkId)");
                                        }
                                    });
                                    debugger
                                    //添加li的鼠标移进移出事件
                                ...
                                if (event.keyCode > 40 || event.keyCode === 8 || event.keyCode === 32) {
                                    //先前有值，
                                    if (value2.linkId && model.get(value2.linkId)) {
                                        changeFlag = true;
                                    }
                                    selectedItem = -1;
                                    if (value2.linkId) {
                                        model.setValue(value2.linkId, "");
                                    }
                                    if (value2.thinkUrl) {
                                        $.ajax({
                                            type: "POST",
                                    ...
                                    } else if (value2.thinkFunc) {
                                        handleThinkFunc(eval(value2.thinkFunc + "('" + $tvalue + "')"));
                                    }
                                    ...
                                    function handleThinkFunc(lxJson) {
                                        ...
                                        if (lxJson && ((maxTrLength = lxJson.length) > 0)) {
                                            ...
                                            if (maxTrLength > 0) {
                                                ...
                                                thinkDiv.width($(t).outerWidth());
                                                thinkDiv.css("left", $(t).offset().left + "px");
                                                thinkDiv.css("top", ($(t).height() + $(t).offset().top) + "px");
                                                thinkDiv.show();
                                            }
                                        } else {
                                            thinkUl.empty();
                ...
                if (event.keyCode === 13) {
                    ...
                    if (value2.linkId) {
                        model.setValue(value2.linkId, trLinkId);
                    }
                    thinkUl.empty();
                    thinkDiv.hide();
                    if (value2.changeFunc) {
                        eval(value2.changeFunc + "(model,trLinkName,trLinkId)");
                    }
                    event.preventDefault();
                }
                ...
                if (thinkDiv && !haverFlag) {
                    //失焦事件
                    thinkDiv.hide();
                    if (value2.linkId && (!model.get(value2.linkId) && model.get(value2.linkId) !== 0 )) {
                        domObj.val("");
                    }
                    if (changeFlag) {
                        if (value2.changeFunc) {
